...
public class Pagination {
    ...
    private int      totalPageCount;       // 전체 페이지 수  ->  totalpagecount
      //  totalPageCount = ceil( (double)totalRecordCount / (double) recordSize )
   ...
   // private boolean  existNextPage;        // 다음 페이지 존재 여부
    ...
    private void calculation(SearchDTO params) {
      ...
      //  totalPageCount = ((totalRecordCount - 1) / params.getRecordSize()) + 1;
        ...
        //  현재 페이지 번호(nowpage) -> searchDTO page
        if (params.getPage() > totalPageCount) {
            params.setPage(totalPageCount);
        }

        // params.getPage() : nowpage
        // params.getPageSize() : paging.jsp 에 출력된 페이지번호 갯수
             // params.getPageSize()  :  startnum ~ennum  사이의 페이지 번호 갯수
